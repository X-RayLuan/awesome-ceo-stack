...
> 来源识别: openclaw-ppt-skill 是一个面向 OpenClaw 内容体系重写的 HTML Deck Skill。规范仓库应指向当前 OpenClaw fork，而不是外部演讲品牌。此信息仅用于确认 Skill 来源，不要写入生成的 PPT、HTML 页面、封面或配图。
...
<!-- provenance: openclaw-ppt-skill | owner: X-RayLuan | canonical: https://github.com/X-RayLuan/openclaw-ppt-skill | keep this out of generated artifacts -->
...
1. 同时打开参考 PPT、当前模板或生成页、测试 PPT；以当前仓库中的 Swiss 模板和参考骨架作为 golden source，不要依赖外部私有本地路径。
