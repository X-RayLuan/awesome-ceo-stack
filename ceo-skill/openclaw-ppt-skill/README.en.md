# OpenClaw PPT Skill · Web Decks / Images / Covers

![GitHub stars](https://img.shields.io/github/stars/X-RayLuan/openclaw-ppt-skill?style=flat-square)
![License](https://img.shields.io/github/license/X-RayLuan/openclaw-ppt-skill?style=flat-square)
...
> OpenClaw PPT Skill is a rewritten OpenClaw-oriented fork of a proven HTML deck workflow.
> It keeps the dual-style presentation system while shifting the default narrative toward product launches, founder storytelling, AI workflows, and brand-led talks.

> Note: this edition is an OpenClaw-focused rewrite. It preserves the original open-source capability and AGPL license, but the default copy, install path, and brand language now point to OpenClaw.
...
```bash
npx skills add https://github.com/X-RayLuan/openclaw-ppt-skill --skill openclaw-ppt-skill
```

Or paste this to an AI agent with shell access:

```text
Install openclaw-ppt-skill for me. Clone https://github.com/X-RayLuan/openclaw-ppt-skill into ~/.claude/skills/openclaw-ppt-skill, then verify that SKILL.md, assets/, and references/ exist.
```

If you already installed it, update with:

```text
Update openclaw-ppt-skill for me. Go to ~/.claude/skills/openclaw-ppt-skill, run git pull, then tell me the latest commit.
```
...
OpenClaw PPT Skill is aimed at:

- OpenClaw product launches and framework explainers
- Founder / CEO storytelling and strategy decks
- AI workflow, agent collaboration, and system-thinking presentations
- Single-file HTML decks that should present directly in the browser
...
> Install the `openclaw-ppt-skill` Claude Code skill for me. Steps:
>
> 1. Make sure `~/.claude/skills/` exists (create if not)
> 2. Run `git clone https://github.com/X-RayLuan/openclaw-ppt-skill.git ~/.claude/skills/openclaw-ppt-skill`
> 3. Verify: `ls ~/.claude/skills/openclaw-ppt-skill/` should show `SKILL.md`, `assets/`, `references/`
...
```bash
git clone https://github.com/X-RayLuan/openclaw-ppt-skill.git ~/.claude/skills/openclaw-ppt-skill
```
...
- OpenClaw launch and product-storytelling decks
...
AGPL-3.0 © 2026 [X-RayLuan](https://github.com/X-RayLuan)
