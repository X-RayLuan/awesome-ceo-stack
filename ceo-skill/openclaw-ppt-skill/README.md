...
> OpenClaw PPT Skill 是基于成熟 HTML Deck 工作流重新整理的 OpenClaw 版本,重点服务于产品发布、方法论表达、AI 叙事和品牌化演讲场景。
> 这个版本保留了原项目的双风格能力,同时改写成更适合 OpenClaw 内容体系的默认语境。

> 注:当前版本是面向 OpenClaw 的二次改写版,保留原始开源能力与 AGPL 许可,但默认文案、安装方式和品牌语义已经切换到 OpenClaw。
...
```text
帮我安装 openclaw-ppt-skill。请把 https://github.com/X-RayLuan/openclaw-ppt-skill 克隆到 ~/.claude/skills/openclaw-ppt-skill,安装完成后检查 SKILL.md、assets/、references/ 是否存在。
```

已经安装过的话,用这段话更新:

```text
帮我更新 openclaw-ppt-skill。请进入 ~/.claude/skills/openclaw-ppt-skill 执行 git pull,然后告诉我当前最新 commit。
```
...
OpenClaw PPT Skill 面向这些内容场景:

- OpenClaw 产品发布和方法论表达
- 创始人叙事、CEO 分享、战略解读
- AI 工作流、Agent 协作、系统化知识输出
- 需要“浏览器直接演示”的单文件 HTML deck
...
> 帮我安装 `openclaw-ppt-skill` 这个 Claude Code skill。请按下面步骤做:
>
> 1. 确保 `~/.claude/skills/` 目录存在(不存在就创建)
> 2. 执行 `git clone https://github.com/X-RayLuan/openclaw-ppt-skill.git ~/.claude/skills/openclaw-ppt-skill`
> 3. 验证:`ls ~/.claude/skills/openclaw-ppt-skill/` 应该看到 `SKILL.md`、`assets/`、`references/` 三项
